...
export async function GET(request: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  // them. Checked before any DB call — no query is worth spending on
  // traffic we're about to redirect away without recording anything.
  ...
  // Card lookup and the rate-limit count don't depend on each other — run
  // them together instead of paying for two sequential round-trips before
  // the redirect. This is the most latency-sensitive route in the app
  // (every physical NFC tap goes through it), so trimming a DB round-trip
  // here matters more than it would on an authenticated admin page.
  const [{ data: card }, rateLimitResult] = await Promise.all([
    admin
      .from("nfc_cards")
  ...
  if (ipHash && (rateLimitResult.count ?? 0) >= 20) {
