...
export async function requestDemo(
  ...
  if (salesInbox) {
    await sendTransactionalEmail({
      ...
      html: `
        <p><strong>Nombre:</strong> ${escapeHtml(parsed.data.fullName)}</p>
        <p><strong>Empresa:</strong> ${escapeHtml(parsed.data.company)}</p>
        <p><strong>Correo:</strong> ${escapeHtml(parsed.data.email)}</p>
        <p><strong>Teléfono:</strong> ${escapeHtml(parsed.data.phone || "—")}</p>
        <p><strong>Mensaje:</strong> ${escapeHtml(parsed.data.message || "—")}</p>
      `,
    });
  }
